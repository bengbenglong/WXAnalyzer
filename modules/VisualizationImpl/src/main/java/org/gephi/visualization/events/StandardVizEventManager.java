...
public class StandardVizEventManager implements VizEventManager {
    ...
    public void mouseLeftClick() {
        //此处为我的代码，左击节点直接显示节点的详细信息,只显示一个节点，忽略其他节点
            ShowDetail showDetail=Lookup.getDefault().lookup(ShowDetail.class);
            List<NodeModel> nodeModel=engine.getSelectedNodes();
            if(nodeModel.size()>0){
                showDetail.showDetail(nodeModel.get(0).getNode());
            }
            
        
            
        //Node Left click 节点左击
