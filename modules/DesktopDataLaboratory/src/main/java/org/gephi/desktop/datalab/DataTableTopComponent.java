...
import edu.seu.desktop.datalab.ButtomInfo;
import edu.seu.desktop.datalab.HelpPanel;
...
@TopComponent.OpenActionRegistration(displayName = "节点参数",
...
public class DataTableTopComponent extends TopComponent implements AWTEventListener, DataTablesEventListener {
    ...
    private final GraphController graphCtrl;
    ...
    public DataTableTopComponent() {
        ...
        graphCtrl = Lookup.getDefault().lookup(GraphController.class);
        Workspace workspace = projectCtrl.getCurrentWorkspace();
        if (workspace == null) {
            clearAll();
        } else {
        ...
        buttonInfoPanel.add(new ButtomInfo());
        initEvents();
    ...
    private void initAction(){
        ...
        helpButton.addActionListener(new ActionListener(){
            @Override
            public void actionPerformed(ActionEvent e){
                DialogDescriptor dd = new DialogDescriptor(new HelpPanel(),"参数说明");
    ...
    private void activateWorkspace(Workspace workspace){
        ...
        graphModel = graphCtrl.getGraphModel(workspace);
        nodesTableObserver = graphCtrl.getGraphModel(workspace).getNodeTable().createTableObserver(false);
        edgesTableObserver = graphCtrl.getGraphModel(workspace).getEdgeTable().createTableObserver(false);
    ...
    private void initEvents() {
        //Workspace Listener
        projectCtrl.addWorkspaceListener(new WorkspaceListener() {
        ...
        if (projectCtrl.getCurrentWorkspace() != null) {
            activateWorkspace(projectCtrl.getCurrentWorkspace());
            setAutoRefreshEnabled(true);
        }
    ...
    private void refreshTable() {
        ...
        refreshAppliedFilter();
        //=========重新将table的宽度传入以便调整每列列宽==================
        nodeTable.setTableComponentWidth(tableScrollPane.getWidth()-tableScrollPane.getVerticalScrollBar().getWidth());
        edgeTable.setTableComponentWidth(tableScrollPane.getWidth()-tableScrollPane.getVerticalScrollBar().getWidth());
    }
    ...
    private String prepareTableExportFileName() {
        ...
        ProjectInformation projectInfo = projectCtrl.getCurrentProject().getLookup().lookup(ProjectInformation.class);
        if (projectInfo.hasFile()) {
            fileName = removeFileNameExtension(projectInfo.getFileName());
        }
        WorkspaceProvider wp = projectCtrl.getCurrentProject().getLookup().lookup(WorkspaceProvider.class);
        if (wp.getWorkspaces().length > 1 || fileName == null) {
            ...
            WorkspaceInformation workspaceInfo = projectCtrl.getCurrentWorkspace().getLookup().lookup(WorkspaceInformation.class);
...
//    }
    ...
    private void initComponents() {
        ...
        org.openide.awt.Mnemonics.setLocalizedText(executeButton, "计算中心性"); // NOI18N
        ...
        controlToolbar.add(executeButton);

        helpButton.setIcon(new javax.swing.ImageIcon(getClass().getResource("/edu/seu/desktop/datalab/image/help.png"))); // NOI18N
        org.openide.awt.Mnemonics.setLocalizedText(helpButton, org.openide.util.NbBundle.getMessage(DataTableTopComponent.class, "DataTableTopComponent.helpButton.text")); // NOI18N
        helpButton.setFocusable(false);
        helpButton.setHorizontalTextPosition(javax.swing.SwingConstants.CENTER);
        helpButton.setVerticalTextPosition(javax.swing.SwingConstants.BOTTOM);
        controlToolbar.add(helpButton);
        ...
        filterTextField.setPreferredSize(new java.awt.Dimension(150, 25));
        controlToolbar.add(filterTextField);

        columnComboBox.setMaximumSize(new java.awt.Dimension(2000, 20));
        columnComboBox.setPreferredSize(new java.awt.Dimension(120, 25));
        ...
        buttonInfoPanel.setBackground(new java.awt.Color(204, 0, 255));
        buttonInfoPanel.setLayout(new java.awt.BorderLayout());
        gridBagConstraints = new java.awt.GridBagConstraints();
        gridBagConstraints.gridx = 0;
        ...
        add(buttonInfoPanel, gridBagConstraints);
    ...
    private void initLookAndFeel(){
       //========look&feel========
        try {
            ...
            java.util.logging.Logger.getLogger(DataTableTopComponent.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        }
        //========look&feel========
