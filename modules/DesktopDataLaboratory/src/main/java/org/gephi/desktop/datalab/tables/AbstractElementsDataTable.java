...
public abstract class AbstractElementsDataTable<T extends Element> {
    ...
    //=========我的=========
    private int tableComponentWidth;
    //======================
    ...
    public AbstractElementsDataTable() {
        ...
        table.setAutoResizeMode(org.jdesktop.swingx.JXTable.AUTO_RESIZE_OFF);
        sparkLinesRenderers = new ArrayList<>();
    ...
    public void refreshModel(T[] elements, Column[] cols, GraphModel graphModel, DataTablesModel dataTablesModel) {
        ...
        for (Column c : cols) {
            //我的代码：如果列名称为Id，则确保该列在第一行
            if(c.getTitle().equals("Id")){
                columns.add(0, new AttributeDataColumn<T>(attributeColumnsController, c));
            }else{
    ...
    public void setTableComponentWidth(int width){
        this.tableComponentWidth=width;
    }
    
    
    //我的方法，根据列名和列内容来重新调整每列的宽度
    private synchronized void resetTableColumnWidth(){
        ...
        int tableWidth=tableComponentWidth;
