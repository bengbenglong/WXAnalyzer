...
public class EigenvectorCentrality implements Statistics, LongTask {
    ...
    public static final String EIGENVECTOR = "特征向量";
    ...
    private Column initializeAttributeColunms(GraphModel graphModel) {
        ...
        if (eigenCol == null) {
            eigenCol = nodeTable.addColumn(EIGENVECTOR, "特征向量", Double.class, new Double(0));
        }
        return eigenCol;
    }
    ...
    private void saveCalculatedValues(Graph hgraph, Column attributeColumn, HashMap<Integer, Node> indicies,
        ...
        for (int i = 0; i < N; i++) {
            ...
            s.setAttribute(attributeColumn, (double)Double.parseDouble(df.format(eigCenrtalities[i])));
        }
    }
