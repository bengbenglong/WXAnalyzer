...
public class Degree implements Statistics, LongTask {
    ...
    public static final String INDEGREE = "入度";
    public static final String OUTDEGREE = "出度";
    public static final String DEGREE = "度";
    public static final String AVERAGE_DEGREE = "平均度";
    ...
    private void initializeAttributeColunms(GraphModel graphModel) {
        ...
        if (isDirected) {
            if (!nodeTable.hasColumn(INDEGREE)) {
                nodeTable.addColumn(INDEGREE, "入度", Integer.class, 0);
            }
            if (!nodeTable.hasColumn(OUTDEGREE)) {
                nodeTable.addColumn(OUTDEGREE, "出度", Integer.class, 0);
            }
        }
        if (!nodeTable.hasColumn(DEGREE)) {
            nodeTable.addColumn(DEGREE, "度", Integer.class, 0);
        }
    }
