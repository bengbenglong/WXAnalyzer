...
public class GraphDistance implements Statistics, LongTask {
    ...
    public static final String BETWEENNESS = "介数";
    public static final String CLOSENESS = "接近度";
    public static final String ECCENTRICITY = "离心率";
    /**
     *      */
    ...
    private void initializeAttributeColunms(GraphModel graphModel) {
        ...
        if (!nodeTable.hasColumn(ECCENTRICITY)) {
            nodeTable.addColumn(ECCENTRICITY, "离心率", Double.class, new Double(0));
        }
        if (!nodeTable.hasColumn(CLOSENESS)) {
            nodeTable.addColumn(CLOSENESS, "接近度", Double.class, new Double(0));
        }
        if (!nodeTable.hasColumn(BETWEENNESS)) {
            nodeTable.addColumn(BETWEENNESS, "介数", Double.class, new Double(0));
        }
    }
     ...
     private void saveCalculatedValues(Graph hgraph, HashMap<Node, Integer> indicies,
        ...
        for (Node s : hgraph.getNodes()) {
            ...
            s.setAttribute(ECCENTRICITY, (double)Double.parseDouble(df.format(nodeEccentricity[s_index])));
            s.setAttribute(CLOSENESS, (double)Double.parseDouble(df.format(nodeCloseness[s_index])));
            s.setAttribute(BETWEENNESS, (double)Double.parseDouble(df.format(nodeBetweenness[s_index])));
        }
    }
