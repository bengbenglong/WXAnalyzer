...
public class ForceAtlas2 implements Layout {
    ...
    private double edgeWeightInfluence;//边的权重影响
    private double jitterTolerance;//容差（速度）
    private double scalingRatio;//缩放
    private double gravity;//重力
    private double speed;//速度
    private double speedEfficiency;
    private boolean outboundAttractionDistribution;//劝阻hubs？
    private boolean adjustSizes;//防止重叠
    private boolean barnesHutOptimize;//近似斥力
    private double barnesHutTheta;//近似
    private boolean linLogMode;//linLog模式
    private boolean strongGravityMode;//更强的重力
    ...
    public ForceAtlas2(ForceAtlas2Builder layoutBuilder) {
        ...
        this.threadCount = Math.min(4, Math.max(1, Runtime.getRuntime().availableProcessors() - 2));
    }
    ...
    public void resetPropertiesValues() {
        ...
        // 调音
        if (nodesCount >= 50) {
            setScalingRatio(10.0);//缩放
        } else {
            setScalingRatio(15.0);
        }
        setStrongGravityMode(false);
        setGravity(1.);

        // 行为替代
        setOutboundAttractionDistribution(true);
        setLinLogMode(true);
        setAdjustSizes(true);
        setEdgeWeightInfluence(1.);

        // 性能
        setJitterTolerance(0.5d);//容差（速度）越小摆动越少，速度会更慢但精度更高
        if (nodesCount >= 1000) {
            setBarnesHutOptimize(true);//近似斥力
        } else {
            setBarnesHutOptimize(false);
        }
        setBarnesHutTheta(1.2);//近似
        setThreadsCount(Math.max(1, Runtime.getRuntime().availableProcessors() - 1));//线程数量
    }
